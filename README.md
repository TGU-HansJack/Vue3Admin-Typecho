...
> 说明：以下为赞助/付费的额外模块；其中“规划中/可定制”项不一定已在当前版本实现，可按需排期实现，交付以 Release 为准。

已支持 / 可开通：

- AI 内容助手：摘要、翻译、对话、写作辅助（可选）
- 数据库工具：文章库增强（检索/统计）、系统库维护（备份/还原/优化）
- API 能力：数据统计 API、数据导出接口、Webhook/第三方对接（可选）
- 媒体库 Pro：图片压缩、WebP/AVIF 转换、水印、批量处理

规划中 / 可定制：

- 高级数据看板：来源/设备/阅读时长/趋势分析
- 对象存储与 CDN：S3/OSS/COS 集成、CDN 刷新
- 权限与安全：细粒度角色权限、操作审计、2FA/登录保护
- 自动化任务：定时备份、自动清理、sitemap/RSS 增强
