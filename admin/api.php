 ...
 * @return mixed
 */
function v3a_http_post_json(string $url, array $payload = [], array $headers = [], int $timeout = 10)
{
    $timeout = max(3, min(45, (int) $timeout));
...
function v3a_host_from_url(string $url): string
{
    $url = trim($url);
    if ($url === '') {
        return '';
...
function v3a_domain_match(string $host, string $rule): bool
{
    ...
    if (strpos($rule, '*.') === 0) {
        ...
        return $suffix !== '' && strlen($host) > strlen($suffix) && substr($host, -strlen($suffix)) === $suffix;
    }

    if (strpos($rule, '.') === 0) {
        return strlen($host) > strlen($rule) && substr($host, -strlen($rule)) === $rule;
    }
...
try {
    ...
    if ($do === 'sponsor.list') {
        ...
        $domain = v3a_string($request->get('domain', ''), '');
        if ($domain === '') {
            $payload = v3a_payload();
            $domain = v3a_string($payload['domain'] ?? '', '');
        }
        if ($domain === '') {
            try {
                $domain = v3a_host_from_url((string) ($options->siteUrl ?? ''));
            } catch (\Throwable $e) {
                $domain = '';
            }
        }
        ...
        if ($domain === '') {
            v3a_exit_json(400, null, 'Missing domain');
        }

        $verifyApi = '';
        try {
            $pluginOptions = $options->plugin('Vue3Admin');
            $verifyApi = trim((string) ($pluginOptions->sponsorVerifyApi ?? ''));
        } catch (\Throwable $e) {
            $verifyApi = '';
        }
        if ($verifyApi === '') {
            $verifyApi = 'https://craft.hansjack.com/usr/themes/classic-22/ai-agent/license-api.php';
        }
        if (!preg_match('/^https?:\/\//i', $verifyApi)) {
            v3a_exit_json(500, null, 'Invalid sponsor verify api');
        }
        ...
        $remote = null;
        try {
            $remote = v3a_http_post_json($verifyApi, [
                'action' => 'list',
                'domain' => $domain,
            ], $headers, 18);
        } catch (\Throwable $e) {
            ...
            ], $e->getMessage() !== '' ? $e->getMessage() : '授权服务不可用');
        }

        $remoteCode = (int) ($remote['code'] ?? 500);
        $remoteMessage = trim((string) ($remote['message'] ?? ''));
        $remoteData = is_array($remote['data'] ?? null) ? $remote['data'] : [];
        $verified = !empty($remoteData['verified']) ? 1 : 0;

        if ($remoteCode !== 0 || !$verified) {
            v3a_exit_json(0, [
                'verified' => 0,
                'domain' => $domain,
                'items' => [],
                'meta' => [
                    'verifyApi' => $verifyApi,
                    'checkedAt' => time(),
                    'message' => $remoteMessage !== '' ? $remoteMessage : '当前域名未授权',
                ],
            ]);
        }
        ...
        $itemsRaw = [];
        if (isset($remoteData['items']) && is_array($remoteData['items'])) {
            $itemsRaw = $remoteData['items'];
        } elseif (isset($remoteData['projects']) && is_array($remoteData['projects'])) {
            $itemsRaw = $remoteData['projects'];
        }
        ...
        foreach ($itemsRaw as $idx => $it) {
            ...
            $name = $pickStr($it, ['name', 'title', 'projectName'], '');
            $type = strtolower($pickStr($it, ['projectType', 'type'], 'page'));
            if ($type === '页面型') {
                $type = 'page';
            } elseif ($type === '功能型') {
                $type = 'feature';
            }
            if (!in_array($type, ['page', 'feature'], true)) {
                $type = 'page';
            }

            $link = $pickStr($it, ['link', 'url', 'zip', 'download'], '');
            $desc = $pickStr($it, ['description', 'desc', 'intro'], '');
            $author = $pickStr($it, ['author'], '');
            $version = $pickStr($it, ['version'], '');
            $readme = $pickStr($it, ['readme', 'readmeUrl', 'docs', 'docsUrl'], '');
            $typecho = $pickAny($it, ['typecho', 'typechoVersion', 'typechoVersions', 'supportTypecho']);

            $id = $pickStr($it, ['id', 'slug', 'key'], '');
            if ($id === '') {
                $id = substr(md5($name . '|' . $link . '|' . (string) $idx), 0, 12);
            }

            $slug = strtolower($pickStr($it, ['slug', 'id', 'key'], ''));
            $slug = preg_replace('/[^a-z0-9_-]+/i', '-', $slug);
            $slug = trim((string) $slug, '-_');
            if ($slug === '') {
                $slug = 'sponsor-' . substr(md5($id . '|' . $name), 0, 8);
            }

            $canInstall = ($type === 'page' && $link !== '');
            $installed = isset($installedPages[$slug]);

            $items[] = [
                'id' => $id,
                'slug' => $slug,
                'name' => $name !== '' ? $name : ('项目' . (string) ($idx + 1)),
                'projectType' => $type,
                'link' => $link,
                'description' => $desc,
                'author' => $author,
        ...
        v3a_exit_json(0, [
            ...
            'meta' => [
                'verifyApi' => $verifyApi,
                'checkedAt' => time(),
                'sourceText' => (string) ($remoteData['sourceText'] ?? '授权服务'),
                'message' => $remoteMessage,
            ],
        ]);
    }

    if ($do === 'sponsor.install') {
        if (!$request->isPost()) {
            v3a_exit_json(405, null, 'Method Not Allowed');
        }
        v3a_security_protect($security, $request);
        ...
        $slug = strtolower(v3a_string($payload['slug'] ?? '', ''));
        $projectType = strtolower(v3a_string($payload['projectType'] ?? ($payload['type'] ?? ''), ''));
        $link = v3a_string($payload['link'] ?? '', '');
        $domain = strtolower(v3a_string($payload['domain'] ?? '', ''));
        if ($domain === '') {
            try {
                $domain = v3a_host_from_url((string) ($options->siteUrl ?? ''));
            } catch (\Throwable $e) {
        ...
        if ($projectType === '页面型') {
            $projectType = 'page';
        } elseif ($projectType === '功能型') {
            $projectType = 'feature';
        }

        if ($slug === '') {
            $slug = strtolower(v3a_string($payload['id'] ?? '', ''));
        }
        $slug = preg_replace('/[^a-z0-9_-]+/i', '-', (string) $slug);
        $slug = trim((string) $slug, '-_');
        if ($slug === '') {
            $slug = 'sponsor-' . substr(md5($name . '|' . $link), 0, 8);
        }
        ...
        if (!in_array($projectType, ['page', 'feature'], true)) {
            v3a_exit_json(400, null, 'Invalid project type');
        }

        if ($projectType !== 'page') {
            v3a_exit_json(501, null, '功能型项目暂未开放安装');
        }

        if ($link === '') {
            v3a_exit_json(400, null, 'Missing project link');
        }
        ...
        try {
            $check = v3a_http_post_json($verifyApi, [
                'action' => 'verify',
                'domain' => $domain,
            ], [
                'Accept' => 'application/json',
            ...
            $checkCode = (int) ($check['code'] ?? 500);
            $checkData = is_array($check['data'] ?? null) ? $check['data'] : [];
            if ($checkCode !== 0 || empty($checkData['verified'])) {
                $msg = trim((string) ($check['message'] ?? '当前域名未授权'));
                v3a_exit_json(403, null, $msg !== '' ? $msg : '当前域名未授权');
            }
        } catch (\Throwable $e) {
            v3a_exit_json(502, null, $e->getMessage() !== '' ? $e->getMessage() : '授权校验失败');
        }
        ...
        try {
            ...
            $tmpDir = $tmpRoot . DIRECTORY_SEPARATOR . 'v3a_sponsor_' . date('Ymd_His') . '_' . substr(md5((string) mt_rand()), 0, 8);
            $zipPath = $tmpDir . DIRECTORY_SEPARATOR . 'package.zip';
            $extractDir = $tmpDir . DIRECTORY_SEPARATOR . 'extract';
            v3a_mkdir_p($extractDir);
            ...
            v3a_http_download_to_file($link, $zipPath, $downloadHeaders, 80);
            if (!is_file($zipPath) || (int) (@filesize($zipPath) ?: 0) < 256) {
            ...
            if (is_array($dirs) && count($dirs) === 1) {
                $pkgRoot = (string) ($dirs[0] ?? $extractDir);
            }

            $sourceDir = '';
            $directBySlug = rtrim($pkgRoot, "/\\") . DIRECTORY_SEPARATOR . $slug;
            if (is_dir($directBySlug) && is_file($directBySlug . DIRECTORY_SEPARATOR . 'index.php')) {
                $sourceDir = $directBySlug;
            }
            if ($sourceDir === '' && is_file(rtrim($pkgRoot, "/\\") . DIRECTORY_SEPARATOR . 'index.php')) {
                $sourceDir = $pkgRoot;
            }
            if ($sourceDir === '') {
                try {
                    ...
                    foreach ($it as $item) {
                        ...
                        $idxFile = rtrim($candidate, "/\\") . DIRECTORY_SEPARATOR . 'index.php';
                        if (!is_file($idxFile)) {
                            continue;
                        }
                        $len = strlen((string) $candidate);
                        if ($len < $bestLen) {
                            $best = $candidate;
                            $bestLen = $len;
                        }
            ...
            v3a_copy_directory($sourceDir, $dest);

            if (!is_file($dest . DIRECTORY_SEPARATOR . 'index.php')) {
        ...
        try {
            ...
            if (is_array($decoded)) {
                foreach ($decoded as $it) {
                    ...
                    $itSlug = strtolower(trim((string) ($it['slug'] ?? '')));
                    $itTitle = trim((string) ($it['title'] ?? ''));
                    $itUrl = trim((string) ($it['url'] ?? ''));
                    if ($itSlug === '' || $itTitle === '' || $itUrl === '') {
                        continue;
                    }
                    $pages[] = [
                        'slug' => $itSlug,
        ...
        foreach ($pages as $i => $it) {
            if (strtolower((string) ($it['slug'] ?? '')) !== $slug) {
                continue;
            }
            $pages[$i] = [
                'slug' => $slug,
        ...
        $encoded = json_encode($pages, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        if (!is_string($encoded) || $encoded === '') {
            v3a_exit_json(500, null, 'Save failed');
        }

        v3a_upsert_option($db, 'v3a_sponsor_pages', $encoded, 0);
